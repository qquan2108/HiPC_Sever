  ...
  try {
    ...
    if (req.body.variants !== undefined) {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: "variants phải là JSON mảng hợp lệ" });
      }
...
exports.getProductById = async (req, res) => {
  try {
    ...
    if (item.category_id?._id) {
      const tpl = await TsktProduct.findOne({
    ...
    for (const row of rows) {
      try {
        if (!row.name || !row.category_id || !row.price) {
          errors.push(
            `Row skipped: Missing required fields (name, category_id, price)`
          );
          continue;
        }
        ...
        const idCond =
          rawId && mongoose.Types.ObjectId.isValid(rawId)
            ? { _id: rawId }
            : null;
        const dupFilter = idCond
          ? { $or: [idCond, { name: row.name }] }
          : { name: row.name };
          ...
          try {
            const parsedVar =
              typeof row.variants === "string"
                ...
                : row.variants;
            if (Array.isArray(parsedVar))
              variants = normalizeVariants(parsedVar);
          } catch (e) {
...
exports.filterProductsByKeyword = async (req, res) => {
  try {
    ...
    const validProducts = products.filter(
      (p) => p._id && mongoose.Types.ObjectId.isValid(p._id)
    );
